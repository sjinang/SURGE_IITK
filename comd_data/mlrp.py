...
while ind < 1:
    ...
    cols[12] = [(float(i)-min(cols[12]))/(max(cols[12])-min(cols[12])) for i in cols[12]]
    cols[13] = [(float(i)-min(cols[13]))/(max(cols[13])-min(cols[13])) for i in cols[13]]
    ...
    X_train, X_test, y_train, y_test = train_test_split(X, Y, test_size=0.2, random_state=rd.randrange(1000)) 
    ...
    name = ['i','j','k','nx','ny','nz','PAPI_L1_TCM','PAPI_L2_TCM','PAPI_L1_LDM','PAPI_L2_LDM','PAPI_BR_PRC','PAPI_RES_STL','PAPI_DP_OPS','PAPI_TOT_CYC']
    rows = np.array(rows)
    data = rows
    df = pd.DataFrame(rows, columns=['i','j','k','nx','ny','nz','PAPI_L1_TCM','PAPI_L2_TCM','PAPI_L1_LDM','PAPI_L2_LDM','PAPI_BR_PRC','PAPI_RES_STL','PAPI_DP_OPS','PAPI_TOT_CYC'])
...
#print "median absolute percentage error =",tmdabse/10.0 *100
